...
class StoreProduct extends SwatDBDataObject
{
     ...
     * @param bool        $limiting whether or not to exclude items unavailable in
     ...
     *                            used.
     *
     * @return bool true if and only if this product is available in the
     *              specified region
     */
    public function isAvailableInRegion(?StoreRegion $region = null)
    {
        ...
        if (isset($this->is_available[$region->id])) {
            ...
            $sql = sprintf(
                'select count(product) from AvailableProductView
    ...
    protected function getSerializablePrivateProperties()
    {
        return array_merge(
            ...
            ['region', 'limit_by_region', 'is_available']
        );
    }
    ...
    protected function loadItems()
    {
        ...
        if ($this->region === null) {
            ...
            $sql = sprintf($sql, $this->db->quote($this->id, 'integer'));
            $items = call_user_func(
                [$wrapper, 'loadSetFromDBWithRegion'],
                $this->db,
                $sql,
                $this->region,
    ...
    protected function loadPath()
    {
        ...
        if ($this->hasInternalValue('path')
            ...
            $sql = sprintf(
                'select getCategoryPath(primary_category)
     ...
     *
     * For a region-aware way to load related products, see {@link * StoreProduct::getRelatedProducts()}
     */
     ...
     * To load only available collection products, see {@link * StoreProduct::getVisibleCollectionProducts}.
     */
