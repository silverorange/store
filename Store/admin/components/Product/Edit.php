...
class StoreProductEdit extends AdminDBEdit
{
    ...
    protected function initCatalogFlydown()
    {
        if ($this->app->getInstance() === null) {
            ...
            $where = sprintf(
                'Catalog.id in (select catalog from
				CatalogInstanceBinding where instance = %s)',
                $this->app->db->quote(
                    $this->app->getInstance()->id,
                    'integer'
                )
            );
        }
    ...
    protected function validateShortname($shortname)
    {
        ...
        $sql = sprintf(
            'select shortname from Product where id %s %s',
        ...
        if ($this->id === null || $old_shortname != $shortname) {
            $sql = sprintf(
                'select clone_of from Catalog where id %s %s',
            ...
            $sql = sprintf(
                'select Catalog.id, Catalog.clone_of from Product
    ...
    protected function saveDBData(): void
    {
        ...
        if ($category !== null && $this->id === null) {
            $sql = sprintf(
                'insert into CategoryProductBinding
				(category, product) values (%s, %s)',
                $category,
                $this->product->id
            );
    ...
    protected function smartDefaultCatalog()
    {
        ...
        $catalog_selector->setState(SiteApplication::initVar(
            'catalog',
            null,
        ...
        if ($catalog_selector->catalog !== null) {
            ...
            $row = SwatDB::queryRow($this->app->db, sprintf(
                $sql,
    ...
    protected function loadAttributes()
    {
        $attribute_values = SwatDB::queryColumn(
            $this->app->db,
            'ProductAttributeBinding',
            'attribute',
            'product',
