...
class StoreCatalogClone extends AdminDBEdit
{
    ...
    protected function saveDBData(): void
    {
        ...
        $clone_id = SwatDB::executeStoredProcOne(
            $this->app->db,
            'cloneCatalog',
            [
        ...
        if ($clone_id == -1) {
            $message = new SwatMessage(
                sprintf(
                    Store::_('The %s “%s” could not be cloned.'),
                    Store::_('catalog'),
                    $title
                ),
                'system-error'
            ...
            $message = new SwatMessage(
                sprintf(
                    Store::_('The %s “%s” has been cloned.'),
                    Store::_('catalog'),
                    $title
                )
            );
    ...
    protected function buildInternal()
    {
        ...
        if ($row->clone_id !== null) {
            throw new AdminNoAccessException(
                sprintf(
                    Store::_('Cannot clone a %s that is already a clone.'),
                    Store::_('catalog')
                )
            );
        }
