...
class StoreItemQuantityDiscount extends AdminIndex
{
    ...
    protected function initInternal()
    {
        ...
        $this->ui->loadFromXML(__DIR__ . '/quantity-discount.xml');
    ...
    protected function getItemRow()
    {
        if ($this->item === null) {
            ...
            $sql = $this->getItemSql();
            $this->item = SwatDB::queryRow($this->app->db, sprintf(
                $sql,
                $regions_select,
                $regions_join,
    ...
    private function addNewQuantityDiscounts()
    {
        ...
        $fields = ['integer:item', 'integer:quantity'];

        $region_fields = ['integer:quantity_discount', 'integer:region',
            'decimal:price'];
        ...
        foreach ($replicators as $replicator_id) {
            if ($this->validateRow($input_row, $replicator_id)) {
                ...
                $values = ['item' => $this->id, 'quantity' => $quantity];

                $discount_id = SwatDB::insertRow(
                    $this->app->db,
                    'QuantityDiscount',
                    $fields,
                    $values,
    ...
    private function validateRow($input_row, $replicator_id)
    {
        ...
        if ($valid) {
            ...
            $quantity = $input_row->getWidget('quantity', $replicator_id);
            $sql = sprintf(
                'select count(id) from QuantityDiscount
    ...
    protected function buildNavBar()
    {
        ...
        $this->navbar->addEntry(new SwatNavBarEntry(sprintf(
            Store::_('Quantity Discounts for SKU %s'),
            $item_row->sku
        )));
    }
