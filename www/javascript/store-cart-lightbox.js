...
function StoreCartLightbox(available_entry_count, all_entry_count) {
  ...
  this.entries_added_event = new YAHOO.util.CustomEvent('entries_added', this);

  this.entry_removed_event = new YAHOO.util.CustomEvent('entry_removed', this);

  this.cart_empty_event = new YAHOO.util.CustomEvent('cart_empty', this);

  this.open_event = new YAHOO.util.CustomEvent('cart_open', this);

  this.close_event = new YAHOO.util.CustomEvent('cart_close', this);
...
StoreCartLightbox.prototype.init = function () {
  ...
  if (cart_links.length > 0) {
    YAHOO.util.Event.on(cart_links, 'click', this.handleLinkClick, this, true);
  }
  ...
  // mini-cart itself
  YAHOO.util.Event.on(this.mini_cart, 'click', function (e) {
  ...
  var close_links = YAHOO.util.Dom.getElementsByClassName(
    'close-lightbox',
    null,
    this.mini_cart
  );
...
StoreCartLightbox.prototype.addEntries = function (
  ...
  this.setContent('<h3>' + StoreCartLightbox.submit_message + '</h3>');
...
StoreCartLightbox.prototype.activateLinks = function () {
  ...
  if (remove_buttons.length !== 0) {
    YAHOO.util.Event.on(remove_buttons, 'click', this.removeEntry, this, true);
  }
  ...
  if (close_buttons.length !== 0) {
    YAHOO.util.Event.on(close_buttons, 'click', this.close, this, true);
  }

  if (document.getElementById('store_cart_lightbox_close_button')) {
    YAHOO.util.Event.on(
      'store_cart_lightbox_close_button',
      'click',
      this.close,
      this,
      true
...
StoreCartLightbox.prototype.getContainerTop = function (contents) {
  ...
  var container_height =
    this.mini_cart.offsetHeight - this.content.offsetHeight + content_height;

  return Math.max(
    (YAHOO.util.Dom.getViewportHeight() - container_height) / 2,
    0
  );
};
...
StoreCartLightbox.prototype.removeRow = function (tr) {
  ...
  animation.onComplete.subscribe(function () {
    ...
    if (that.current_delete == current_delete) {
      that.setContentWithAnimation(that.content.innerHTML);
    }
  });
...
StoreCartLightbox.prototype.close = function (e) {
  ...
  if (this.status == 'open') {
    ...
    if (YAHOO.env.ua.mobile) {
      ...
      this.main_animation.onComplete.subscribe(
        function () {
...
StoreCartLightbox.prototype.updateItemCount = function (item_count) {
  ...
  if (item_count === 1) {
    ...
    message =
      ' ' +
      StoreCartLightbox.item_count_message_plural.replace(/%s/, item_count);
  }
