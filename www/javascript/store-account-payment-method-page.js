function StoreAccountPaymentMethodPage(
  id,
  inception_date_ids,
  issue_number_ids
) {
  ...
  for (var i = 0; i < inception_date_ids.length; i++) {
    var type = document.getElementById('payment_type_' + inception_date_ids[i]);

    if (type) this.inception_date_types.push(type);
  }

  for (var i = 0; i < issue_number_ids.length; i++) {
    var type = document.getElementById('payment_type_' + issue_number_ids[i]);

    if (type) this.issue_number_types.push(type);
  }
  ...
  var payment_type_options = document.getElementsByName('payment_type');
...
StoreAccountPaymentMethodPage.prototype.updateFields = function () {
  if (this.isInceptionDateSensitive()) this.sensitizeInceptionDate();
  else this.desensitizeInceptionDate();

  if (this.isIssueNumberSensitive()) this.sensitizeIssueNumber();
  else this.desensitizeIssueNumber();
};
