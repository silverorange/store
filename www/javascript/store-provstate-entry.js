...
StoreProvStateEntry.prototype.setCountryFlydown = function (
  country_flydown_id
) {
...
StoreProvStateEntry.prototype.handleProvStateChange = function (e) {
  if (this.country_flydown_id) {
    var country_id = this.country.options[this.country.selectedIndex].value;
...
StoreProvStateEntry.prototype.updateProvState = function () {
  ...
  if (this.data[country_id]) {
    ...
    if (this.data[country_id].visible === false) {
      ...
      option.appendChild(
        document.createTextNode(this.data[country_id].select_title)
      );
      ...
      for (var i = 0; i < provstates.length; i++) {
        ...
        if (
          typeof this.provstate_id[country_id] != 'undefined' &&
...
StoreProvStateEntry.prototype.updateProvStateLabel = function (
  data,
  for_value
) {
  ...
  this.label.appendChild(document.createTextNode(data.field_title));

  if (data.required) {
    var required = document.createElement('span');
    required.className = 'swat-required';
    required.appendChild(
      document.createTextNode(StoreProvStateEntry.required_text)
    );
