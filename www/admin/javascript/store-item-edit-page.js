...
function StoreItemEditPage(form_id, price_replicators) {
  ...
  for (var i = 0; i < price_replicators.length; i++) {
    this.price_replicators[i] = this.getItemRegionReplicator(
      price_replicators[i]
    );
  }
  ...
  YAHOO.util.Event.addListener(
    form,
    'submit',
    StoreItemEditPage.handleSubmit,
    this
  );
}
...
function StoreItemRegionReplicator(id) {
  ...
  YAHOO.util.Event.addListener(
    this.enabled,
    'click',
    StoreItemRegionReplicator.handleClick,
    this
  );

  // initialize
  if (this.enabled.checked) this.sensitize(false);
  else this.desensitize();
}

StoreItemRegionReplicator.handleClick = function (event, replicator) {
  if (replicator.enabled.checked) replicator.sensitize(true);
  else replicator.desensitize();
};
